...
| Method | Path | Meaning |
|---|---|---|
| POST | `/api/jobs` | `{url, quality?, localVerification?}` creates a queued intent; `localVerification` creates a paused Chrome-extension handoff job |
...
| POST | `/api/jobs/:id/verified-source` | authenticated one-time Chrome-extension media handoff; source details are forwarded only to the worker’s memory |
